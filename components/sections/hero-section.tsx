...
const HERO_BACKGROUND_BLOBS = [
  {
    className:
      "absolute top-[-10%] left-[-5%] h-[60%] w-[50%] rounded-full bg-[#0e122c] opacity-60 blur-[90px]",
    animation: "blob-float-1 20s ease-in-out infinite",
  },
  {
    className:
      "absolute top-[10%] left-[20%] h-[70%] w-[60%] rounded-full bg-[#080C25] opacity-70 blur-[90px]",
    animation: "blob-float-2 25s ease-in-out infinite",
  },
  {
    className:
      "absolute top-[5%] right-[-5%] h-[50%] w-[40%] rounded-full bg-[#060534] opacity-50 blur-[90px]",
    animation: "blob-float-3 22s ease-in-out infinite",
  },
  {
    className:
      "absolute right-[-5%] bottom-[-10%] h-[60%] w-[55%] rounded-full bg-[#121735] opacity-65 blur-[90px]",
    animation: "blob-float-1 28s ease-in-out infinite reverse",
  },
  {
    className:
      "absolute top-[30%] left-[30%] h-[40%] w-[40%] rounded-full bg-[#06091E] opacity-80 blur-[90px]",
    ...
    <>
      <div className="absolute inset-0 z-0 bg-[#0E1331]">
        {HERO_BACKGROUND_BLOBS.map(({ className, animation }) => (
          <div key={className} className={className} style={{ animation }} />
        ))}
      </div>
...
function HeroActions({ isMac }: { isMac: boolean }) {
  return (
    ...
    >
      {isMac ? <MacDownloadButton /> : <WindowsComingSoonButton />}
      <GitHubSourceButton />
    </motion.div>
  );
}
...
function WindowsComingSoonButton() {
  return (
    <ShortcutPillButton
